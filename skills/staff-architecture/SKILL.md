...
description: >
  ...
  Use when Codex needs to assess cross-layer design, package boundaries,
  security architecture, authorization architecture, database scalability,
  CI/CD readiness, observability, or long-term maintainability in backend/,
  frontend/, and shared/.
---

Adapted from `/.kiro/skills/staff-architecture.md` for Codex skill invocation.
...
maintainability â€” always grounded in the **real, current state of the
...
answering â€” this summary is a starting orientation, not a substitute for
reading the files.
...
- Layers: `routes/ â†’ controller/ â†’ service/ â†’ repositories/ â†’ db/`
...
- Rate limiting: in-memory `Map` only â€” does not survive restarts, does not
  scale across instances
- Authorization: **inconsistent** â€” `verifyToken` authenticates, but ownership
  checks are not applied uniformly across controllers
- Validation: manual, centralized in `validateRequest.ts` â€” large, growing file
...
- Platform abstraction: `platform/` exists with `storage/`, `network/`,
  `backButton/`, `isNative.ts` â€” but `isNative()` always returns `false` and
...
```text
shared/
â”œâ”€â”€ assets/       logos, flags, fonts, images
â”œâ”€â”€ domains/      auth, user, account, category, creditCard, tag,
â”‚                 subcategory, transaction, feedback, phone
â”œâ”€â”€ enums/        24 enum files covering routes, storage, auth, UI, HTTP,
â”‚                 log, language, transaction, upload, etc.
â”œâ”€â”€ errors/       error-codes.ts, error-payload.ts
â”œâ”€â”€ fields/       field-keys.ts, field-label-map.ts
â”œâ”€â”€ i18n/         locales (en-US, pt-BR, es-ES), mappings, types, translate.ts
â””â”€â”€ types/        format.types.ts, pagination.types.ts
```

Key contracts:
- `ErrorCode` â€” machine-stable error identifiers returned by the backend
- `FieldKey` â€” typed field identifiers for validation messages
- `I18nKey = keyof I18nCatalog` â€” derived from the `en-US` canonical catalog
- Non-canonical locales (`pt-BR`, `es-ES`) use `satisfies` against the
  canonical shape â€” missing keys fail TypeScript
- Runtime default locale is `pt-BR`, not `en-US` (canonical type source is
  `en-US` â€” this is a documented divergence)
...
- `withTransaction()` casts `tx` to `typeof db` to work around a Drizzle typing
  incompatibility â€” this is an acknowledged workaround in the source comment
- `TransactionService` applies signed monetary deltas to account/card balances
  inside database transactions â€” this is the most critical financial consistency
...
- Whether any new code is placing application logic inside `shared/` (it must
  not â€” `shared/` is for contracts, enums, i18n, and assets only)
- Whether the dependency direction is respected:
  `pages â†’ services â†’ api â†’ platform/utils`, never the reverse
...
- Is the `route â†’ controller â†’ service â†’ repository` chain respected?
...
rule â€” what the system must enforce and why â€” not to specify the
...
Neither overrides the other â€” they are complementary and must be read together.
...
- Is rate limiting applied to new auth-adjacent endpoints? (Known: current rate
  limiting is in-memory only â€” flag this for any multi-instance deployment)
...
- Are DaisyUI utility classes appearing directly in `src/pages/**`? (Forbidden
  by the architectural guideline â€” flag divergences)
...
contract surface. Every structural addition or change to `shared/` â€” new
domain types, new enums, new `ErrorCode` entries, new `FieldKey` entries, new
shared utility types, new assets â€” requires your review and approval before it
is merged.

Other agents have scoped roles within `shared/`:
- `staff-ux-writing` owns the content and language governance of
  `shared/i18n/` â€” i18n key organization, locale catalog quality, and
...
- `staff-design-system` owns the subset of `shared/enums/*` that directly
  defines shared UI system semantics â€” `ButtonVariant`, `ButtonSize`,
...
- Is `ResourceKey` being extended? (It is a legacy layer â€” new code should not
  depend on it)
...
- Is there a typed, validated config module for the backend? (Currently absent
  â€” flag any new env var added without validation)
...
- Is `express-session` still unused? (It is in `package.json` but not in
  source â€” flag if it is activated without a security review)
...
- Are migrations being applied in the correct sequence in CI? (Currently no CI
  migration gate exists â€” flag this)
...
- Are env vars validated at startup? (Currently absent â€” flag any new env var
...
- Is there any error tracking integration? (Sentry, Datadog, etc. â€” not found)
