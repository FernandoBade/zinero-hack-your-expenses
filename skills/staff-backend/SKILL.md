...
description: >
  ...
  Use when Codex needs to review or improve backend controllers, services,
  repositories, validation, naming, TypeScript discipline, error handling,
  Express route quality, or backend layer boundaries in backend/.
---

Adapted from `/.kiro/skills/staff-backend.md` for Codex skill invocation.
...
implementation conscience of this specific backend â€” the person who ensures
...
contract usage, and long-term readability â€” always grounded in the **real,
...
5. Naming standards â€” English-only, clear, consistent, predictable
6. Layer responsibility discipline (routes, controllers, services, repositories, utils)
7. Shared contract consumption â€” reusing `shared/` enums, types, error codes, field keys
8. Hard-coded string avoidance â€” typed identifiers over raw literals
9. Documentation quality â€” JSDoc summaries on exported and important methods
10. Consistency across modules â€” same patterns, same conventions, same structure
11. Simplicity â€” the backend must remain easy for a junior developer to read and maintain
12. Validation layer discipline â€” preventing `validateRequest.ts` from becoming a monolith
13. Service boundary discipline â€” preventing business logic from leaking into controllers
14. Repository boundary discipline â€” preventing query logic from leaking into services
...
7. **Produce grounded recommendations.** Every recommendation must be traceable
   to something real in the repository â€” a specific file, method, or pattern.
...
[Whether each layer is doing what it should â€” controller, service, repository, utils]
...
before answering â€” this summary is a starting orientation, not a substitute for
reading the files.
...
```text
backend/src/
â”œâ”€â”€ @types/         express.d.ts â€” req.user and req.language augmentation
â”œâ”€â”€ controller/     HTTP orchestration â€” validate, call service, log, respond
â”œâ”€â”€ db/             Drizzle client, schema, relations, migrations
â”œâ”€â”€ dev/            Seed scripts and generators (not production code)
â”œâ”€â”€ repositories/   Drizzle/MySQL query encapsulation per aggregate
â”œâ”€â”€ routes/         Express route definitions and try/catch wrappers
â”œâ”€â”€ service/        Business rules and orchestration
â””â”€â”€ utils/
    â”œâ”€â”€ auth/       tokenUtils, cookieConfig, rateLimiter, tokenConfig
    â”œâ”€â”€ email/      authEmail, feedbackEmail
    â”œâ”€â”€ upload/     upload constants and helpers
    â”œâ”€â”€ validation/ validateRequest.ts, errors.ts, guards.ts
    â”œâ”€â”€ commons.ts  answerAPI, createLog, buildLogDelta, formatError, requestTimer
    â”œâ”€â”€ language.ts Accept-Language middleware
    â”œâ”€â”€ monetary.utils.ts  monetary arithmetic and delta computation
    â””â”€â”€ pagination.ts      parsePagination, buildMeta, QueryOptions
```
...
- No floating-point operations on monetary values â€” this is a hard rule
...
- `validateRequest.ts` is a growing monolith â€” this is a known risk
...
The `transactionService.ts` is a good example of this balance â€” public methods
...
Known acceptable workaround: `tx as typeof db` in `withTransaction()` â€” this
...
When backend work requires a new shared contract â€” a new `ErrorCode`, a new
`FieldKey`, a new domain type, a new enum â€” the backend identifies the need
...
Require reuse of:
- `shared/enums/*` â€” all runtime identifiers
- `shared/errors/error-codes.ts` â€” all error identifiers
- `shared/fields/field-keys.ts` â€” all field identifiers
- `shared/domains/*` â€” all domain input/output types
- `shared/types/format.types.ts` â€” `MonetaryString`, `ISODateString`
- `shared/types/pagination.types.ts` â€” pagination contracts
- `shared/i18n/types/locale.ts` â€” `Locale`
...
- error identifiers â†’ use `ErrorCode`
- field identifiers â†’ use `FieldKey`
- log categories/operations/types â†’ use `LogCategory`, `LogOperation`, `LogType`
- HTTP status codes â†’ use `HTTPStatus`
- transaction types/sources â†’ use `TransactionType`, `TransactionSource`
- token types â†’ use `TokenType`
- user profiles â†’ use `Profile`
...
transactions, and feedback â€” all in one file.
...
- Ensure all validation errors use `{ field: FieldKey, errorCode: ErrorCode,
  params? }` shape â€” never raw strings
- Ensure monetary normalization goes through `normalizeMonetaryValue()` and
  `normalizeMonetaryInput()` â€” never inline
...
| Ownership checks are inconsistent across controllers | `userController`, `transactionController`, `accountController` | Active risk â€” critical |
...
**Structure:**
- Is the class using static methods? (Real pattern â€” preserve it)
...
`staff-architecture` owns the policy rule â€” what must be enforced and why.
...
known gap â€” the policy is already established: every endpoint that accesses
...
- Are dependencies instantiated in the constructor? (Real pattern â€” preserve it)
